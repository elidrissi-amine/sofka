...
pub struct DrainConfig {
    ...
    /// Evict pods that no controller will recreate: pods with no controlling
    /// owner reference (`--force`). The reference is read from the pod, not
    /// resolved, so a pod whose owner has since been deleted still counts as
    /// managed — `kubectl` looks the owner up and this does not.
    pub force: bool,
}
