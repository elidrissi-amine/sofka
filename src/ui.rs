...
fn draw_confirm(frame: &mut Frame, app: &App, area: Rect) {
    ...
    // Display width, not scalar count: a double-width name or key label wraps
    // earlier than a count of chars would predict, which would leave the
    // dialog a row short and clip the text it was sized to hold.
    let longest = app
        .confirm_label
        .split('\n')
    ...
    let rows: usize = app
        ...
        .split('\n')
        .chain(std::iter::once(&*hint))
        .map(|line| UnicodeWidthStr::width(line).div_ceil(inner).max(1))
        .sum();
