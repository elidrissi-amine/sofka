...
fn daemonset_pod(pod: &Pod) -> bool {
    // Only the *controlling* owner counts, as `kubectl` reads it: a pod that
    // merely references a DaemonSet without being controlled by one is an
    // ordinary pod, and has to face the unmanaged-pod rule rather than being
    // skipped as somebody else's business.
    pod.metadata
        .owner_references
        .as_ref()
        .and_then(|owners| owners.iter().find(|owner| owner.controller == Some(true)))
        .is_some_and(|owner| owner.kind.eq_ignore_ascii_case("DaemonSet"))
}
